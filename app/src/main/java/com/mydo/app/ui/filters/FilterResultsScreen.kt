...
fun FilterResultsScreen(viewModel: FilterResultsViewModel, navController: NavController) {
    ...
    LaunchedEffect(viewModel) {
        viewModel.events.collect { event ->
            when (event) {
                is FilterResultsEvent.TaskCompleted -> {
                    scope.launch {
    ...
    Scaffold(snackbarHost = { SnackbarHost(snackbarHostState) }) { padding ->
        when (val state = uiState) {
            FilterResultsUiState.Loading -> MydoLoadingState(message = "Running filter\u2026", modifier = Modifier.fillMaxSize().padding(padding))
            is FilterResultsUiState.Error -> MydoErrorState(
                title = "Unable to run filter",
                message = state.message,
                modifier = Modifier.fillMaxSize().padding(padding),
            )
            is FilterResultsUiState.Ready -> {
                Column(modifier = Modifier.fillMaxSize().padding(padding).padding(MydoSpacing.screenMargin)) {
