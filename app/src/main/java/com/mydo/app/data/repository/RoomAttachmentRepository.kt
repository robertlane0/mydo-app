...
class RoomAttachmentRepository(private val db: MydoDatabase) : AttachmentRepository {
    ...
    override fun observeByTask(taskId: UUID): Flow<AppResult<List<Attachment>>> =
        dao.observeByTask(taskId.toUUIDString())
            .map<List<AttachmentEntity>, AppResult<List<Attachment>>> { list -> AppResult.Success(list.map { it.toDomain() }) }
            .catch { e -> emit(AppResult.Failure(DatabaseError("db_error", "Failed to load attachments", e))) }

    override suspend fun create(attachment: Attachment): AppResult<Unit> = try {
        dao.insert(attachment.toEntity())
        AppResult.Success(Unit)
    } catch (e: Exception) {
        AppResult.Failure(DatabaseError("db_error", "Failed to attach file", e))
    }
    ...
    override suspend fun delete(id: UUID): AppResult<Unit> = try {
        ...
        AppResult.Failure(DatabaseError("db_error", "Failed to remove attachment", e))
    }
