...
class RoomLabelRepository(private val db: MydoDatabase) : LabelRepository {
    ...
    override fun observeAll(): Flow<AppResult<List<Label>>> =
        dao.observeAll().map<List<LabelEntity>, AppResult<List<Label>>> { list -> AppResult.Success(list.map { it.toDomain() }) }
            .catch { e -> emit(AppResult.Failure(DatabaseError("db_error", "Failed to load labels", e))) }

    override fun observeByTask(taskId: UUID): Flow<AppResult<List<Label>>> =
        dao.observeByTask(taskId.toUUIDString())
            .map<List<LabelEntity>, AppResult<List<Label>>> { list -> AppResult.Success(list.map { it.toDomain() }) }
            .catch { e -> emit(AppResult.Failure(DatabaseError("db_error", "Failed to load labels", e))) }

    override suspend fun getByTask(taskId: UUID): AppResult<List<Label>> = try {
        AppResult.Success(dao.getByTask(taskId.toUUIDString()).map { it.toDomain() })
    } catch (e: Exception) {
        AppResult.Failure(DatabaseError("db_error", "Failed to load labels", e))
    }
    ...
    override suspend fun findByName(name: String): AppResult<Label?> = try {
        AppResult.Success(dao.findByName(name)?.toDomain())
    } catch (e: Exception) {
        AppResult.Failure(DatabaseError("db_error", "Failed to look up label", e))
    }
    ...
    override suspend fun assignToTask(taskId: UUID, labelId: UUID): AppResult<Unit> = try {
        dao.insertTaskLabel(TaskLabelCrossRef(taskId.toUUIDString(), labelId.toUUIDString()))
        AppResult.Success(Unit)
    } catch (e: Exception) {
        AppResult.Failure(DatabaseError("db_error", "Failed to apply label", e))
    }

    override suspend fun unassignFromTask(taskId: UUID, labelId: UUID): AppResult<Unit> = try {
        dao.deleteTaskLabel(taskId.toUUIDString(), labelId.toUUIDString())
        AppResult.Success(Unit)
    } catch (e: Exception) {
        AppResult.Failure(DatabaseError("db_error", "Failed to remove label", e))
    }
    ...
    override suspend fun countTasks(labelId: UUID): AppResult<Int> = try {
        AppResult.Success(dao.countByLabel(labelId.toUUIDString()))
    } catch (e: Exception) {
        AppResult.Failure(DatabaseError("db_error", "Failed to count label tasks", e))
    }
}
