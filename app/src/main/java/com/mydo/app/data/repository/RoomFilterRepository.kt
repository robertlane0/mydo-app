...
class RoomFilterRepository(private val db: MydoDatabase) : FilterRepository {
    ...
    override fun observeAll(): Flow<AppResult<List<Filter>>> =
        dao.observeAll().map<List<FilterEntity>, AppResult<List<Filter>>> { list ->
            AppResult.Success(list.map { it.toDomain() })
        }.catch { e ->
            emit(AppResult.Failure(DatabaseError("db_error", "Failed to load filters", e)))
        }

    override fun observeFavorites(): Flow<AppResult<List<Filter>>> =
        dao.observeFavorites().map<List<FilterEntity>, AppResult<List<Filter>>> { list ->
            AppResult.Success(list.map { it.toDomain() })
        }.catch { e ->
            emit(AppResult.Failure(DatabaseError("db_error", "Failed to load favorite filters", e)))
        }
